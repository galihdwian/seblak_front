...
export default function CleaningData(props) {
    const { show } = props
    ...
    const loadData = async () => {
        setLoading(true)
        const result = await dispatch(LoggedAxios({ method: 'get', endpoint: `/cleaning`, params: { show: show, key: state.input.key }, payloads: {} }))
            .then((resp) => {
                return { ...resp.payload }
            }, (error) => {
                if (error.response) {
    ...
    const submitStart = async () => {
        ...
        const result = await dispatch(LoggedAxios({ method: method, endpoint: `/cleaning`, params: {}, payloads: payloads }))
            .then((resp) => {
                return { ...resp.payload }
            }, (error) => {
                if (error.response) {
                    ...
                    <div className="space-y-2">
                            ...
                            state.input.action === 'start' && <>
                                ...
                                <div>
                                    <div className="mb-2 block">
                                        <Label htmlFor="receiver">Waktu Start</Label>
                                    </div>
                                    <TextInput
                                        id="time"
                        ...
                        }
                        {
                            state.input.action !== 'start' && <>
                                <div>
                                    <div className="mb-2 block">
                                        <Label htmlFor="receiver">Waktu End</Label>
                                    </div>
                                    <TextInput
                                        id="time"
            ...
            <div className="flex flex-col divide-y min-h-full">
                <div className="p-4 bg-white">
                    <div className="flex flex-row justify-between">
                        <h1 className="font-bold text-2xl">Data Cleaning</h1>
                    </div>
                </div>
                ...
                <form className="flex flex-wrap gap-2" onSubmit={handelSearch}>
                    ...
                    </form>
                    {
                        (checkedList.length && checkedFilter !== 1) ? <div>
                            <button className="bg-blue-500 text-white rounded px-2 py-2 flex gap-1 items-center" onClick={() => handleOpenModalStart_Multiple()}><FaCheck />Mulai Cleaning</button>
                        </div> : ''
                    }
                    {
                        (checkedList.length && checkedFilter === 1) ? <div>
                            <button className="bg-emerald-500 text-white rounded px-2 py-2 flex gap-1 items-center" onClick={() => handleOpenModalEnd_Multiple()}><FaCheck />Selesai Cleaning</button>
                        </div> : ''
                    }
                                        ...
                                        .map((v, k) => {
                                            return <tr className={["border-t border-b text-xs", ((checkedList.indexOf(v.id) >= 0) || (state.input.selected_id === v.id) ? 'bg-yellow-200' : '')].join(" ")} key={k}>
                                                ...
                                                <td className="p-1 px-1">
                                                    <LabelStatusCleaning status={v.status} />
                                                </td>
                                                <td className="p-1">
                                                    <div className="flex flex-col gap-2">
                                                        {
                                                            (v.status === 0) && (checkedList.length === 0) && <button className="bg-blue-500 text-white rounded px-2 flex gap-1 items-center" onClick={() => handleOpenModalStart(v.id)}><FaCheck />Mulai Cleaning</button>
                                                        }
                                                        <span>
