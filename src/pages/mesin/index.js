...
import { LabelStatusMesin } from "@/app/_bekaya/SeblakHelper";
...
export default function MesinData(props) {
    ...
    const submitDelete = async (id) => {
        setLoading(true)
        const result = await dispatch(LoggedAxios({ method: 'delete', endpoint: `/mesin/${id}`,  params:{},payloads:{} }))
        .then((resp) => {
          return {...resp.payload}
        }, (error) => {
          if (error.response) {
    ...
    return (
        <>
            ...
            <Modal size="lg" show={openModal} onClose={() => setOpenModal(false)}>
                <ModalHeader><div className="text-xl font-medium text-gray-900 dark:text-white">Mesin Baru</div></ModalHeader>
                <ModalBody>
                    <MesinForm id={selectedId} loadData={loadData} setOpenModal={setOpenModal} />
                </ModalBody>
            </Modal>
            ...
            <div className="flex flex-col divide-y min-h-full">
                <div className="p-4 bg-white">
                    <div className="flex flex-row justify-between">
                        <h1 className="font-bold text-2xl">Data Mesin</h1>
                        <div>
                        ...
                        <table className="w-full text-xs">
                            <thead>
                                <tr className="border-t border-b">
                                    ...
                                    <th className="px-1 py-2 text-left bg-gray-50">Last Test</th>
                            ...
                            <tbody>
                                {
                                    ...
                                    .filter((v) => {
                                        return ((v.nama + "").toLowerCase().indexOf(state.input.key.toLowerCase()) >= 0)
                                            || ((v.nomor + "").toLowerCase().indexOf(state.input.key.toLowerCase()) >= 0)
                                    })
                                    .map((v, k) => {
                                        return <tr className="border-t border-b text-xs" key={k}>
                                            <td className="py-1">{k + 1}</td>
                                            <td className="py-1">{v.nama}</td>
                                            <td className="py-1">{v.nomor}</td>
                                            <td className="py-1"><LabelStatusMesin status={v.status} /></td>
                                            <td className="py-1"><StripEmpty value={v.last_test} type="datetime" format="DD-MMM-YYYY HH:mm" /></td>
