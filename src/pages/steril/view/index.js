...
export default function SterilView() {
    ...
    const loadModel = async () => {
        setLoading(true)
        const result = await dispatch(LoggedAxios({ method: 'get', endpoint: `/steril/detail/${id}`, params: {}, payloads: {} }))
            .then((resp) => {
                return { ...resp.payload }
            }, (error) => {
                if (error.response) {
        ...
        if (data?.responseCode == '000') {
            setModel(data.data.sterilisasi)
            document.title = `Detail Sterilisasi ${data.data.sterilisasi?.trans_kode}`
        }
        setLoading(false)
    }
    ...
    const submitStatus = async () => {
        ...
        const result = await dispatch(LoggedAxios({ method: 'put', endpoint: `/steril`, params: {}, payloads: payload }))
            .then((resp) => {
                return { ...resp.payload }
            }, (error) => {
                if (error.response) {
                ...
                <ModalBody>
                    {
                        (model.status === 0) && <div className="space-y-6">
                            <h3 className="text-xl font-medium text-gray-900 dark:text-white">Set Steril</h3>
                            <div>
                    ...
                    }
                    {
                        (model.status === 1) && <div className="space-y-6">
                            <h3 className="text-xl font-medium text-gray-900 dark:text-white">Set Selesai sterilisasi</h3>
                            <div>
                                <div className="mb-2 block">
            ...
            <div className="flex flex-col divide-y min-h-full">
                <div className="p-4 bg-white">
                    <div className="flex flex-row justify-between items-center min-h-10">
                        <h1 className="font-bold text-2xl">Detail Sterilisasi {model.trans_kode}</h1>
                        <LabelStatusSteril status={model.status} />
                    </div>
                </div>
                <div className="flex flex-col gap-4 p-4">
                    ...
                    <div className="flex flex-col gap-4">
                        <div className="flex flex-row justify-end">
                            {
                                model.status === 0 && <button onClick={() => setOpenModalStatus(1)} className="rounded-xl bg-blue-600 text-white px-4 py-2 items-center flex flex-row gap-2"><HiCheck /> Mulai Sterilisasi</button>
                            }
                            {
                                model.status === 1 && <button onClick={() => setOpenModalStatus(2)} className="rounded-xl bg-blue-600 text-white px-4 py-2 items-center flex flex-row gap-2"><HiCheck /> Set Selesai Sterilisasi</button>
                            }
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4" >
                        <div className="rounded-xl bg-white p-4 flex flex-col gap-1">
                            <div className="flex flex-row gap-x-3 gap-y-1">
                                <label className="lg:w-5/12 lg:shrink-0 text-xs font-bold">Nama Instrumen</label>
                                <span className="font-bold">{model.iru_nama}</span>
                            </div>
                            <div className="flex flex-row gap-x-3 gap-y-1">
                                <label className="lg:w-5/12 lg:shrink-0 text-xs font-bold">Mesin</label>
                                <span><StripEmpty value={model.msn_nama} /> (<StripEmpty value={model.msn_no} />)</span>
                            </div>
                            <div className="flex flex-row gap-x-3 gap-y-1">
                                <label className="lg:w-5/12 lg:shrink-0 text-xs font-bold">Waktu Mulai Pencucian</label>
                                <span><StripEmpty value={model.start} type="datetime" format="DD MMMM YYYY HH:mm" /></span>
                            </div>
                            <div className="flex flex-row gap-x-3 gap-y-1">
                                <label className="lg:w-5/12 lg:shrink-0 text-xs font-bold">Waktu Akhir Pencucian</label>
                                <span><StripEmpty value={model.end} type="datetime" format="DD MMMM YYYY HH:mm" /></span>
                            </div>
                            <div className="flex flex-row gap-x-3 gap-y-1">
                                <label className="lg:w-5/12 lg:shrink-0 text-xs font-bold">Durasi</label>
                                <span>{CalcuDuration({start: model.start, end: model.end, format: 'HH [menit] : m [detik]'})}</span>
                            </div>
                            <div className="flex flex-row gap-x-3 gap-y-1">
                                <label className="lg:w-5/12 lg:shrink-0 text-xs font-bold">Petugas Pensteril</label>
                                <span><StripEmpty value={model.steril_man} /></span>
                            </div>
                        </div>
