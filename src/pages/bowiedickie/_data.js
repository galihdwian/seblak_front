...
export default function BowiedickData(props) {
    const { show } = props
    ...
    const loadData = async () => {
        ...
        const result = await dispatch(LoggedAxios({ method: 'get', endpoint: '/bowiedick', params: { show: show } }))
            .then((resp) => {
                return { ...resp.payload }
            }, (error) => {
                if (error.response) {
    ...
    const submit = async () => {
        ...
        const result = await dispatch(LoggedAxios({ method: method, endpoint: `/bowiedick/status/${state.input.selected_id}`, params: {}, payloads: payloads }))
            .then((resp) => {
                return { ...resp.payload }
            }, (error) => {
                if (error.response) {
    ...
    const submitExport = async () => {
        ...
        const result = await dispatch(LoggedAxios({ method: method, endpoint: `/bowiedick/export`, params: {}, payloads: payloads, config: { responseType: 'blob' } }))
            .then((resp) => {
                return { ...resp.payload }
            }, (error) => {
                if (error.response) {
    ...
    return (
        <>
            ...
            <Head>
                <title>Data Uji Bowie Dickie</title>
            </Head>
            <div className="flex flex-col divide-y min-h-full">
                <div className="p-4 bg-white">
                    <div className="flex flex-col sm:flex-row gap-2 sm:justify-between min-h-10">
                        <h1 className="font-bold text-2xl">Data Uji Bowie Dickie</h1>
                        <div>
                            <Link className="rounded-xl bg-green-800 text-white px-4 py-2 items-center flex flex-row gap-2" href={`/bowiedickie/new`}><span><HiPlus /></span> Tambah</Link>
                        </div>
                    </div>
                </div>
