...
export default function BowiedickView(props) {
    ...
    const loadModel = async () => {
        setLoading(true)
        const result = await dispatch(LoggedAxios({ method: 'get', endpoint: `/bowiedick/detail/${id}`, params: {}, payloads: {} }))
            .then((resp) => {
                return {...resp.payload}
            }, (error) => {
                if (error.response) {
        ...
        if (data?.responseCode == '000') {
            setModel(data.data.bowie_dick_test)
        }
        setLoading(false)
    }
    ...
    return (
        <>
            <Head>
                <title>Detail Uji Bowie Mesin {model.mesin_nama}</title>
            </Head>
            <div className="flex flex-col divide-y min-h-full">
                <div className="p-4 bg-white">
                    <div className="flex flex-col lg:flex-row gap-2 lg:justify-between min-h-10">
                        <h1 className="font-bold text-2xl">Detail Uji Bowie Mesin {model.mesin_nama}</h1>
                    </div>
                </div>
