...
export default function BowiedickForm() {
    ...
    const submit = async () => {
        ...
        const result = await dispatch(LoggedAxios({ method: method, endpoint: `/bowiedick`,  params:{},payloads:payloads }))
        .then((resp) => {
          return {...resp.payload}
        }, (error) => {
          if (error.response) {
